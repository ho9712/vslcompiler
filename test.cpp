...
static std::string IdentifierStr; //variable
static int NumVal;  // integer
...
static int gettok() {
	...
	if (isalpha(LastChar)) { // variable: （{lc_letter}|{uc_letter}）({lc_letter}|{uc_letter}|{digit})*
	...
	if (isdigit(LastChar)) {//integer:{digit}+
		std::string NumStr;
		do {
			NumStr += LastChar;
			LastChar = getchar();
	...
	if (LastChar == ':' && (LastChar = getchar()) == '=') {//assign_symbol:  ":="	
		LastChar = getchar();
